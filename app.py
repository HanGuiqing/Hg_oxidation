...
with col1:
    ...
    CO2 = st.number_input('CO2 (ppm)', min_value=0.0, value=1000000, step=1, key='CO2_c')
    O2 = st.number_input('O2 (ppm)', min_value=0.0, value=1000000, step=1, key='O2_c')
    SO2 = st.number_input('SO2 (ppm)', min_value=0.0, value=1000000, step=1, key='SO2_c')
    NO = st.number_input('NO (ppm)', min_value=0.0, value=1000000, step=1, key='NO_c')
    Cl2 = st.number_input('Cl2 (ppm)', min_value=0.0, value=1000000, step=1, key='Cl2_c')
    Br2 = st.number_input('Br2 (ppm)', min_value=0.0, value=1000000, step=1, key='Br2_c')
    HCl = st.number_input('HCl (ppm)', min_value=0.0, value=1000000, step=1, key='HCl_c')
    HBr = st.number_input('HBr (ppm)', min_value=0.0, value=1000000, step=1, key='HBr_c')

    st.markdown('</div>', unsafe_allow_html=True)
with col2:
    st.markdown(f'<div class="st-ba">', unsafe_allow_html=True)
    st.markdown('**Operating conditions**')
    T = st.number_input('Tempreature (°C)', min_value=0.0, value=1000.0, step=1, key='Temperature')
    RT = st.number_input('Residence time (s)', min_value=0.0, value=10.0, step=0.1, key='Residence_time')
    FR= st.number_input('Flow rate ()', min_value=0.0, value=87000.0, step=1, key='Flow_rate')
    QR = st.number_input('Quench rate (K/s)', min_value=0.0, value=7000, step=1, key='Q_rate')
